...
var WeaponDB = (function () {
  ...
  async function fetchBySlug(slug) {
    ...
    var { data, error } = await client
      ...
      .eq('slug', slug)
      .single();
    if (error) { console.error('[WeaponDB] fetchBySlug error:', error.message); return null; }
    return data;
  }
  ...
  async function insert(weaponData) {
    ...
    if (!weaponData.slug && weaponData.name) {
      weaponData.slug = weaponData.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
    }
    var { data, error } = await client
      .from('weapons')
      .insert([weaponData])
      .select()
      .single();
    if (error) console.error('[WeaponDB] insert error:', error.message);
    return { data, error };
  }
  ...
  async function update(id, updates) {
    ...
    var { data, error } = await client
      ...
      .select()
      .single();
    if (error) console.error('[WeaponDB] update error:', error.message);
    return { data, error };
  }
  ...
  async function remove(id) {
    var client = SupaDB.getClient();
    if (!client) return { error: { message: 'Not connected' } };
    // Also remove character links
    await unlinkAllCharacters(id);
    var { error } = await client
      .from('weapons')
      .delete()
      .eq('id', id);
    if (error) console.error('[WeaponDB] delete error:', error.message);
    return { error };
  }
  ...
  async function uploadIcon(file, weaponSlug) {
    ...
    if (error) {
      ...
      // Fallback: try character-images bucket
      var result2 = await client.storage
        .from('character-images')
        .upload(filePath, file, { cacheControl: '3600', upsert: true });
      if (result2.error) return { error: result2.error, url: null };
      var { data: urlData } = client.storage.from('character-images').getPublicUrl(filePath);
      return { error: null, url: urlData.publicUrl };
    }
    var { data: urlData } = client.storage.from('weapon-icons').getPublicUrl(filePath);
    return { error: null, url: urlData.publicUrl };
  }

  // ============================================================
  // CHARACTER LINKING - weapon_characters junction table
  // ============================================================
  ...
  async function fetchLinkedCharacters(weaponId) {
    ...
    if (error) { console.error('[WeaponDB] fetchLinkedCharacters error:', error.message); return []; }
    return data || [];
  }
  ...
  async function fetchWeaponsForCharacter(characterId) {
    ...
    var { data, error } = await client
      ...
      .order('priority', { ascending: true });
    if (error) { console.error('[WeaponDB] fetchWeaponsForCharacter error:', error.message); return []; }
    return data || [];
  }
  ...
  async function linkCharacter(weaponId, characterId, opts) {
    ...
    var { data, error } = await client
      ...
      .select()
      .single();
    if (error) console.error('[WeaponDB] linkCharacter error:', error.message);
    return { data, error };
  }
  ...
  async function unlinkCharacter(weaponId, characterId) {
    ...
    var { error } = await client
      ...
      .eq('weapon_id', weaponId)
      .eq('character_id', characterId);
    if (error) console.error('[WeaponDB] unlinkCharacter error:', error.message);
    return { error };
  }
  ...
  async function unlinkAllCharacters(weaponId) {
    var client = SupaDB.getClient();
    if (!client) return;
    await client.from('weapon_characters').delete().eq('weapon_id', weaponId);
  }
  ...
  async function bulkImport(weapons) {
    ...
    var { data, error } = await client
      ...
      .select();
    if (error) console.error('[WeaponDB] bulkImport error:', error.message);
    return { data, error };
  }

  async function exportJSON(weapons) {
    var blob = new Blob([JSON.stringify(weapons, null, 2)], { type: 'application/json' });
    var url = URL.createObjectURL(blob);
    var a = document.createElement('a');
    a.href = url;
    a.download = 'weapons-export-' + new Date().toISOString().slice(0, 10) + '.json';
    a.click();
